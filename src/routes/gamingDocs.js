...
function aggregateSections() {
  ...
  const artwork = { portraitUrl: null, landscapeUrl: null, squareUrl: null, iconUrl: null, all: [] };
  const currentGame = { id: 'valorant', name: 'VALORANT', edition: { id: 'standard', name: 'Standard' }, artwork };
  const summary = { earned: 1, available: 2, known: 2, completionPercent: 50 };
  const progressCases = [
    { case: 'steam-only', status: 'ready', progress: { status: 'available', setCount: 1,
      rarestUnlock: { id: 'steam:570:achievement:FIRST', source: 'steam', kind: 'achievement', rarityPercent: 12.5 },
      sets: [{ id: 'steam:570:achievements', source: 'steam', kind: 'achievement', summary }],
      sources: [{ source: 'steam', providerGameIds: ['570'], status: 'available' }] } },
    { case: 'psn-only', status: 'ready', progress: { status: 'available', setCount: 1,
      rarestUnlock: { id: 'psn:trophy2:NPWR12345_00:trophy:1', source: 'psn', kind: 'trophy', grade: 'bronze', rarityPercent: 2.5 },
      sets: [{ id: 'psn:trophy2:NPWR12345_00', source: 'psn', kind: 'trophy', summary }],
      sources: [{ source: 'psn', providerGameIds: ['PPSA12345_00'], status: 'available' }] } },
    { case: 'matched-steam-and-psn', status: 'ready', progress: { status: 'available', setCount: 2,
      completionCalculation: 'reported-per-progress-set-no-cross-source-merge',
      sets: [{ id: 'psn:trophy2:NPWR12345_00', source: 'psn', kind: 'trophy', summary },
        { id: 'steam:570:achievements', source: 'steam', kind: 'achievement', summary }] } },
    { case: 'partial-provider-failure', status: 'partial', progress: { status: 'partial', setCount: 1,
      sets: [{ id: 'steam:570:achievements', source: 'steam', kind: 'achievement', summary }],
      sources: [{ source: 'steam', providerGameIds: ['570'], status: 'available' },
        { source: 'psn', providerGameIds: ['PPSA12345_00'], status: 'unavailable' }] } },
    { case: 'no-unlock-data', status: 'ready', progress: { status: 'unsupported', setCount: 0, unlockCount: 0,
      rarestUnlock: null, sets: [], sources: [{ source: 'epic', providerGameIds: ['epic-id'], status: 'unsupported' }] } },
  ];
  return [
    ...
    { title: 'Library model', text: 'games are canonical works. Each work contains editions, each edition contains independently played copies, and each copy contains its raw normalized provider observations. The frontend may show one work card, edition cards, or individual platform cards. Curated mappings in config/game-identities.json confirm cross-provider identity. Epic- or Steam-backed Playnite records merge into a unique direct provider title despite cosmetic trademark, accent, punctuation, or capitalization differences. Their observations use role helper-mirror and helperFor identifies the storefront. If no unique direct record exists, the Playnite record stays classified under that storefront rather than becoming a local copy. Other exact title matches remain separate and appear in possibleMatches.' },
    { title: 'Canonical game detail', text: 'GET /aggregate/games/:canonicalGameId is the authoritative page response. game.playtime is the only playtime total the frontend should display or calculate from. activity contains current sessions for this canonical game. progress keeps every Steam achievement set and PlayStation trophy set separate under one provider-neutral contract. Regional PSN title records that resolve to the same service and trophy-set ID appear once, with every title ID retained in providerGameIds and sourceRefs. There is no combined cross-source completion percentage. Each set has its own earned, available, known, and completionPercent values. Steam unlock imageUrl is state-aware: earned achievements prefer unlockedIconUrl, locked achievements prefer lockedIconUrl, and both fall back to iconUrl. Unknown states use the best available provider icon. progress.rarestUnlock is the earned unlock with the lowest reported source-population percentage across the available sets. Epic, Playnite, and Valorant report unsupported for unlocks rather than empty achievements. Failures from unrelated providers do not lower this game response, while a configured matched source that is unavailable appears in progress.sources.', language: 'json', code: JSON.stringify(progressCases, null, 2) },
    { title: 'Artwork model', text: 'Every game and copy has artwork.portraitUrl, artwork.landscapeUrl, artwork.squareUrl, artwork.iconUrl, and artwork.all. The four selected fields are independent and can be null when no source supplies that role. squareUrl is reserved for genuine square promotional artwork suitable for a large tile. Small Steam application and community icons, Playnite application icons, and Epic Android icons only qualify for iconUrl and remain retained in artwork.all. artwork.all retains every distinct safe URL from every source, including logos, backgrounds, characters, and screenshots, plus provider, provider game ID, source role, original type, semantic roles, dimensions, content type, provider metadata, and every source that contributed an identical URL. Only identical URLs are deduplicated. Authoritative provider images are selected ahead of helper mirrors. No generic coverUrl or backgroundUrl alias is returned.' },
    ...
    { title: 'Freshness and errors', text: 'The library returns HTTP 200 when at least one provider is usable and status is ready or partial. HTTP 503 means every source is unavailable. Now playing returns HTTP 200 for playing or offline and HTTP 503 for unknown when no fresh presence source exists. HTTP 401 means a missing or invalid read key. A canonical game lookup returns HTTP 404 when its ID is absent. sources always carries provider freshness so the frontend can explain partial results.' },
    { title: 'Frontend guidance', text: 'For live activity, render every entry in sessions. Do not choose one global winner. Use primarySource for the label and detectedBy for optional attribution. Use artwork.landscapeUrl for wide cards, portraitUrl for poster cards, squareUrl for square tiles, and iconUrl for compact icons. Do not read coverUrl or backgroundUrl. For the library, use the canonical game id for routes and UI state. Treat playtime.status unknown as missing data, not zero. Display work or edition totals only when the product wants a combined view, and keep copy breakdowns available. A complete implementation handoff is linked from the repository documentation.' },
  ];
}
